final String address = "10.0.2.2";
// final String address = "192.168.1.40";
final String port = "8080";

// Authentication URLS
final String signUpURL = "http://$address:$port/api/auth/signup";
final String signInURL = "http://$address:$port/api/auth/signin";

final String pingURL = "http://$address:$port/api/auth/ping";

// Family URLS
final String familyURL = "http://$address:$port/api/family";
final String familyMembersURL = "http://$address:$port/api/family/users";

// List URLS
final String listsURL = "http://$address:$port/api/list";

// Item URLS
final String itemsURL = "http://$address:$port/api/item";

